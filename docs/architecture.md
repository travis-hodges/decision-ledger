...
5. Use **Analysis** to structure a rough request into a reviewed draft frame, or ask a focused question about the entered record. Public web search requires a separate opt-in for each analysis request. Treat generated analysis as a draft for review.
...
| `server/index.mjs`                               | Local, server-side structured intake and SSE analysis streaming     |
| OpenAI Responses API                             | Advisory analysis, calculation tool, optional public search         |

The model does not calculate the official decision score. The deterministic engine owns scoring and creates a snapshot of each evaluation's inputs. Subsequent edits make an older run stale. Structured intake is checked against a JSON Schema and exact quoted excerpts for extracted options and constraints; only a human can apply its question and objective. The AI receives bounded selected fields and source records for analysis; it is instructed to cite source IDs, label uncertainty, and avoid fabricating facts. These controls reduce risk but do not guarantee correctness. A reviewer must verify citations and conclusions.

## Boundaries before operational use

This is an unclassified, single-browser prototype. There is no login, role separation, centralized audit log, persistent database, approved data boundary, or authority to operate. Do not enter sensitive procurement, personnel, export-controlled, or classified material. The analysis API validates incoming decision records, but a production path needs validation on every write path, durable versioned storage, identity and permissions, access logging, citation verification, evaluation fixtures, and deployment in an approved environment. Public web search must be governed by data policy.
